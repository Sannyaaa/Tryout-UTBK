                ...
                <div class="space-y-4">
                    <div class="grid lg:grid-cols-2 gap-3">
                        <div>
                            ...
                            <x-text-input type="text" :value="$order->invoice ?? old('invoice')" name="invoice" id="invoice" placeholder="Masukan Nama Order" required=""/>
                            <x-input-error :messages="$errors->get('invoice')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="final_price" :value="__('Total Price')" />
                            <x-text-input type="text" :value="$order->final_price ?? old('final_price')" name="final_price" id="final_price" placeholder="Masukan Nama Order" required=""/>
                    ...
                    <div class="grid lg:grid-cols-2 gap-3">
                        <div>
                            <x-input-label for="user_id" :value="__('Pembeli')" />
                            <x-text-input type="text" :value="$order->user->name ?? old('user_id')" name="user_id" id="user_id" placeholder="Masukan Nama Order" required=""/>
                            <x-input-error :messages="$errors->get('user_id')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="package_member_id" :value="__('Paket yang Dibeli')" />
                            <x-text-input type="text" :value="$order->package_member->name ?? old('package_member_id')" name="package_member_id" id="package_member_id" placeholder="Masukan Nama Order" required=""/>
                    ...
                    <div class="grid lg:grid-cols-2 gap-3">
                        <div>
                            <x-input-label for="discount_id" :value="__('Mapel')" />
                            <x-select-input id="discount_id" name="discount_id" >
                                <option selected="" disabled>Pilih Mapel</option>
                                @foreach ($vouchers as $voucher)
                                    <option value="{{ $voucher->id }}" {{ old('discount_id', $order->discount_id)  == $voucher->id ? 'selected' : '' }}>{{ $voucher->name }}</option>
                                @endforeach
                            </x-select-input>
                        ...
                        <div>
                            ...
                            <x-select-input id="payment_status" name="payment_status" >
                    ...
                    <div class="flex justify-between">
                        <x-secondary-href href="{{ route('admin.order.index') }}">
                            Kembali
                        </x-secondary-href>
                        <x-primary-button>
                            Edit Order
