...
<a href="{{ $link ?? '#' }}" class=" {{ request()->fullUrlIs(url($link)) ? " text-sky-500 font-medium hover:text-sky-700 bg-sky-400 bg-opacity-5 border-l-4 border-sky-500" : " text-gray-700 hover:bg-sky-400 hover:bg-opacity-5 hover:border-l-4 hover:border-sky-500" }} flex items-center py-2 px-3 text-base rounded-lg transition-all duration-200 group dark:text-gray-200 dark:hover:bg-gray-700">
    <span class=" {{ request()->fullUrlIs(url($link)) ? " hover:text-gray-700  text-white" : " text-gray-100 hover:text-white" }} transition duration-200 group-hover:text-white dark:text-gray-400 dark:group-hover:text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
        {{ $slot }}
    </span>
